...
public final class GeneralPurposeComputer {
    ...
    private void update() {
        ...
        for (int y = 19; y >= 0; --y) {
            for (int x = 9; x >= 0; --x) {
                cells[y][x] = processorAndMemory.read((11 * (2 + y) + x) + 0x0100);
            }
        }
        playfieldFrame.update(playfieldModel);        
        processorAndMemory.write(0x0000, playfieldModel.isLeftPressed() ? 1 : 0);
        processorAndMemory.write(0x0001, playfieldModel.isRightPressed() ? 1 : 0);
        processorAndMemory.write(0x0002, playfieldModel.isDownPressed() ? 1 : 0);
    }
